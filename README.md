...
|---|---|---|---|---|
| gemm 4096³ | 06 matmul | cuBLAS | ~1.0x | ~100% of cuBLAS |
| attention fwd, 4k ctx | 08 | SDPA | ~1.0x | ~100% of SDPA |
| decode, 32k cache | 21 | SDPA | 1.7-1.9x | 90-98% of copy BW |
| transpose 8192² | 09 | torch .T.contiguous() | ~3x | 90-95% of copy BW |
| histogram, 8 hot bins | 10 privatized | 10 naive atomics | ~16x | — (atomics, not BW) |
| int8 gemm 2048³ | 28 | cuBLAS fp16 | 1.8-3x | above fp16 roofline by design |
...
10. File 29, the CUDA one: my block_sum returned the right value only in lane 0 of warp 0 and everyone else normalized by a partial sum. Both the broadcast AND the "all 32 lanes must enter the shuffle" rule bit me in the same function — shuffles are cooperative, a lane can't opt out
